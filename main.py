...
<style>
  ...
  body { font-family: Arial, Helvetica, sans-serif; color: #2b2d38; margin: 0; background: #fff; }
  .page { width: 100%; max-width: 980px; margin: 0 auto; }
  .top { display: grid; grid-template-columns: 210px 1fr; gap: 22px; align-items: center; border-bottom: 4px solid #ef2533; padding-bottom: 14px; }
  .logo { max-width: 200px; max-height: 95px; object-fit: contain; }
  .kicker { color: #ef2533; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; font-size: 13px; }
  h1 { margin: 8px 0 4px; font-size: 32px; line-height: 1.05; color: #171923; }
  .meta { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }
  .pill { border: 1px solid #d9dce5; border-radius: 999px; padding: 6px 11px; font-size: 12px; background: #f8f8fb; }
  .solapa { width: 100%; height: 24px; object-fit: cover; margin: 12px 0 18px; }
  .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
  .card { border: 1px solid #e0e1e8; border-radius: 12px; padding: 14px 16px; background: #fff; box-shadow: 0 1px 4px rgba(0,0,0,.05); break-inside: avoid; }
  .card h2 { color: #ef2533; margin: 0 0 8px; font-size: 15px; text-transform: uppercase; letter-spacing: .03em; border-bottom: 1px solid #f0b2ba; padding-bottom: 5px; }
  p { font-size: 13px; line-height: 1.38; margin: 0 0 7px; }
...
<div class="page">
  <header class="top">
    <img class="logo" src="{{LOGO_URI}}" alt="NANTA">
    <div>
...
def build_html_cards(fields: Dict[str, str], text: str) -> str:
    preferred_sections = [
        "Definición / Posicionamiento",
        "Características",
        "Beneficios",
    ...
    ignored = {
        "Título", "Producto", "Nombre comercial", "Especie", "Subespecie", "Lifestage",
        "Tipo documento", "Texto final", "Texto_final",
    }
    ordered = []
    for field in preferred_sections:
        if clean_text(fields.get(field, "")):
            ordered.append(field)
    for field, value in fields.items():
        if field in ignored or field in ordered:
            continue
        if clean_text(value):
            ordered.append(field)
...
def replace_template_tokens(template: str, variables: Dict[str, str]) -> str:
    """Replace both legacy uppercase tokens and human/lowercase tokens.

    This lets the user create a Canva/HTML-like template with markers such as
    {{PRODUCT}}, {{producto}}, {{Nombre comercial}} or {{modo_empleo}}.
    """
    rendered = template
    for key, value in variables.items():
        ...
        # Also support spaces around token contents, e.g. {{ producto }}.
        for token in candidates:
            rendered = re.sub(r"\{\{\s*" + re.escape(token) + r"\s*\}\}", value, rendered)
    return rendered


def render_nanta_html(doc_type: str, title: str, flat_rows: List[Dict[str, Any]], text: str) -> str:
    ...
    sections_html = build_html_cards(fields, text)

    template_path = TEMPLATES_DIR / "nanta_corporativa.html"
    ...
    variables: Dict[str, str] = {
        ...
        "tipo_ficha": html_lib.escape(doc_type),
        "TITLE": html_lib.escape(title),
        "titulo": html_lib.escape(title),
        "PRODUCT": html_lib.escape(product),
        "producto": html_lib.escape(product),
        ...
        "secciones": sections_html,
        "DATE": date.today().isoformat(),
        "fecha": date.today().isoformat(),
    }
    for field, value in fields.items():
        escaped = html_lib.escape(clean_text(value))
        variables[field] = escaped
        variables[placeholder_key(field)] = escaped
        # For multiline fields, also expose an HTML version.
        variables[f"{placeholder_key(field)}_html"] = section_to_html(value)

    return replace_template_tokens(template, variables)
...
# ---------------------------------------------------------------------------
# Parser de formulaciones tipo Single-Mix / SP:
# ---------------------------------------------------------------------------

def split_sp_blocks(raw_text: str) -> List[str]:
    """Split formulation reports exported as text blocks beginning with ': SP:'."""
    if not raw_text:
        return []
    matches = list(re.finditer(r"(?im)^.*:\s*SP\s*:\s*\S+.*$", raw_text))
    if not matches:
        matches = list(re.finditer(r"(?im)^\s*SP\s*:\s*\S+.*$", raw_text))
    blocks: List[str] = []
    for i, match in enumerate(matches):
...
def parse_sp_line(block: str) -> Tuple[str, str, str]:
    first_lines = "\n".join(block.splitlines()[:4])
    m = re.search(
        r"SP\s*:\s*(?P<spec>\S+)\s+(?P<name>.*?)\s+(?P<pct>\d+(?:[\.,]\d+)?)\s*%,\s*"
        r"(?P<kg>\d+(?:[\.,]\d+)?)\s*Kg.*?(?:Optimal\s+cost\s*:\s*(?P<cost>[-0-9\.,]+))?",
        first_lines,
        flags=re.IGNORECASE | re.DOTALL,
    )
    if not m:
        line = next((clean_text(x).strip(": ") for x in block.splitlines() if "SP" in x.upper()), "Producto sin nombre")
        return "", line[:90], ""
    spec = clean_text(m.group("spec"))
    name = re.sub(r"\s+", " ", clean_text(m.group("name"))).strip(" :-")
    cost = clean_text(m.group("cost") or "")
    return spec, name, cost


def parse_sp_ingredients(block: str) -> pd.DataFrame:
    lines = block.splitlines()
    start = None
    for i, line in enumerate(lines):
        if "INCLUDED RAW MATERIALS" in line.upper():
    ...
    for line in lines[start:]:
        ...
        if not text or set(text) <= {"-"}:
            continue
        if any(marker in upper for marker in ["REJECTED RAW MATERIALS", "NUTRIENT ANALYSIS", "ANALYSIS"]):
            break
        if not re.search(r"^\s*[A-Z]?\d+\s+\d+\.", raw_line):
            continue
        ...
        if parsed is None:
            m = re.match(
                r"^\s*(?P<line_code>[A-Z]?\d+)\s+(?P<material>.+?)\s+"
                r"(?P<pct>-?\d+(?:[\.,]\d+)?)\s+(?P<kilos>-?\d+(?:[\.,]\d+)?)\s+"
                r"(?P<cost>-?\d+(?:[\.,]\d+)?)"
                r"(?:\s+(?P<limit>MAX|MIN|min|max|FIJO|Fijo|fix|FIX))?"
                r"(?:\s+(?P<minimum>\.|-?\d+(?:[\.,]\d+)?))?"
                r"(?:\s+(?P<maximum>\.|-?\d+(?:[\.,]\d+)?))?",
...
def parse_sp_analysis(block: str) -> pd.DataFrame:
    lines = block.splitlines()
    start = None
    for i, line in enumerate(lines):
    ...
    for line in lines[start:]:
        ...
        if not text or set(text) <= {"-"}:
            continue
        if any(marker in upper for marker in ["REJECTED ANALYSIS", "FORMULA SUMMARY", "OPTIMIZATION", "INCLUDED RAW MATERIALS", ": SP:"]):
            break
...
def parse_sp_text(raw_text: str) -> Dict[str, Dict[str, Any]]:
    products: Dict[str, Dict[str, Any]] = {}
    for idx, block in enumerate(split_sp_blocks(raw_text), start=1):
        spec, name, cost = parse_sp_line(block)
        if not name:
            name = f"Producto {idx}"
        ingredients = parse_sp_ingredients(block)
        nutrients = parse_sp_analysis(block)
        display = f"{spec} | {name}" if spec else name
        base_display = display
        n = 2
        while display in products:
...
def load_formula_file(uploaded_file) -> Dict[str, Dict[str, Any]]:
    ...
    products = parse_sp_text(raw_text)
    if products:
        return products
...
def sidebar_uploads() -> Tuple[Dict[str, Dict[str, Any]], pd.DataFrame, pd.DataFrame]:
    ...
    st.sidebar.header("Plantilla corporativa")
    template_ok = (TEMPLATES_DIR / "nanta_corporativa.html").exists()
    logo_ok = (ASSETS_DIR / "Logo1 Nanta.jpg").exists()
    solapa_ok = (ASSETS_DIR / "Solapa rosa.jpg").exists()
    st.sidebar.caption(f"Plantilla HTML: {'OK' if template_ok else 'NO encontrada'}")
    st.sidebar.caption(f"Logo: {'OK' if logo_ok else 'NO encontrado'} · Solapa: {'OK' if solapa_ok else 'NO encontrada'}")
...
def render_downloads(
    ...
    html = render_nanta_html(doc_type, f"{doc_type} - {title_base}", flat_rows, edited_text)

    with st.expander("Vista previa maquetada con plantilla NANTA", expanded=False):
        components.html(html, height=900, scrolling=True)
        st.caption("Esta es la ficha generada con templates/nanta_corporativa.html y assets/Logo1 Nanta.jpg + assets/Solapa rosa.jpg.")
    ...
    with c4:
        st.download_button(
            ...
            help="Ficha maquetada con plantilla corporativa. Se puede abrir en navegador e imprimir/guardar como PDF.",
        )
    with c5:
