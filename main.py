...
# Perfiles de nutrientes persistentes.
NUTRIENT_PROFILES = {
...
def clean_product_name(value: Any, fallback: Any = "") -> str:
    """Devuelve un nombre comercial limpio para mostrar en las fichas."""
...
def table_to_text(df: pd.DataFrame, title: str = "") -> str:
    """Convierte un DataFrame en una tabla de texto legible.

    Evita líneas del tipo "Nutriente: X | Valor: Y" repetidas.
    """
    if df is None or df.empty:
        return ""
    clean_df = df.copy()
    for col in clean_df.columns:
        clean_df[col] = clean_df[col].map(clean_text)
    clean_df = clean_df.replace("", pd.NA).dropna(axis=1, how="all").fillna("")
    if clean_df.empty:
        return ""
    headers = [clean_text(c) for c in clean_df.columns]
    rows = [[clean_text(v) for v in row] for row in clean_df.astype(str).values.tolist()]
    widths = []
    for i, header in enumerate(headers):
        max_row = max([len(r[i]) for r in rows] + [0])
        widths.append(min(max(len(header), max_row), 34))
...
def is_table_separator(line: str) -> bool:
    line = line.strip()
    return bool(line) and set(line) <= {"-", "+", "|", " "} and "-" in line and "|" in line


def split_pipe_table(lines: List[str], start: int) -> Tuple[Optional[List[List[str]]], int]:
    """Detecta tablas de texto generadas por table_to_text."""
    ...
    while i < len(lines) and "|" in lines[i] and lines[i].strip():
        ...
        i += 1
    table = [[clean_text(cell) for cell in line.split("|")] for line in table_lines]
    if len(table) < 2:
        return None, start
    return table, i
...
def text_to_docx_bytes(text: str, title: str) -> bytes:
    ...
    while i < len(lines):
        ...
        if not cleaned:
            doc.add_paragraph("")
        elif cleaned.isupper() and len(cleaned) < 90:
...
def text_to_pdf_bytes(text: str, title: str) -> bytes:
    ...
    while i < len(lines):
        cleaned = lines[i].strip()
        table, next_i = split_pipe_table(lines, i)
        if table and Table is not None and TableStyle is not None:
            pdf_table = Table([[xml_escape(cell) for cell in row] for row in table], repeatRows=1)
            pdf_table.setStyle(TableStyle([
                ("GRID", (0, 0), (-1, -1), 0.25, colors.grey if colors else None),
                ("BACKGROUND", (0, 0), (-1, 0), colors.whitesmoke if colors else None),
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
                ("FONTSIZE", (0, 0), (-1, -1), 8),
            ]))
...
def build_context(
    ...
    selected_nutrients: Dict[str, List[str]],
) -> Dict[str, Any]:
    ctx = dict(manual_values)
    clean_name = clean_product_name(product_data.get("product_name", ""), product_display)
    ctx.setdefault("Producto", product_display)
    ctx["Nombre comercial"] = clean_product_name(ctx.get("Nombre comercial", clean_name), clean_name)
    ...
    commercial_nutrients = selected_nutrients_df(product_data, selected_nutrients.get("commercial", []))
    technical_nutrients = selected_nutrients_df(product_data, selected_nutrients.get("technical", []))
    ctx["Nutrientes seleccionados - FT Comercial"] = table_to_text(commercial_nutrients)
    ctx["Nutrientes seleccionados - FT Calidad-Operaciones y FT Especificaciones"] = table_to_text(technical_nutrients)
    ctx["Nutrientes seleccionados"] = ctx["Nutrientes seleccionados - FT Calidad-Operaciones y FT Especificaciones"]
...
def build_document_text(
    ...
    if include_nutrients:
        nutrient_field = (
            "Nutrientes seleccionados - FT Comercial"
            if doc_type == "FT Comercial"
            else "Nutrientes seleccionados - FT Calidad-Operaciones y FT Especificaciones"
        )
        nutrient_text = field_value(ctx, nutrient_field)
        if nutrient_text:
            lines.append("CONSTITUYENTES ANALÍTICOS")
            lines.append(nutrient_text)
            lines.append("")
            flat_rows.append({
                "Tipo documento": doc_type,
                "Producto": product,
                "Campo": nutrient_field,
                "Valor": nutrient_text,
            })
...
def reset_current_product_state() -> None:
    ...
    for key in list(st.session_state.keys()):
        if key in preserve:
            continue
        if key.startswith(("manual_", "select_", "generated_", "final_text_", "fields_", "_current_product_key")):
...
def select_base_fields(
    ...
    product_key = f"{product_data.get('product_id', '')}|{product_name}|{product_display}"
    if st.session_state.get("_current_product_key") != product_key:
        for key in list(st.session_state.keys()):
            if key.startswith(("manual_", "final_text_")):
                del st.session_state[key]
        st.session_state["generated_docs"] = {}
        st.session_state["_current_product_key"] = product_key
    ...
    if st.button("Pasar opciones elegidas a campos editables", key="apply_selected_texts", use_container_width=True):
        for key, value in proposed_values.items():
    ...
    precauciones = st.text_area("Precauciones de uso / recomendaciones de manejo", height=100, key="manual_precauciones")
...
def get_nutrient_profiles() -> Dict[str, str]:
    """Devuelve los perfiles de nutrientes sin depender de estado externo.

    Esta función evita que la app se caiga si una versión desplegada queda con
    constantes antiguas o si Streamlit conserva estado de una ejecución previa.
    """
    profiles = globals().get("NUTRIENT_PROFILES")
...
def load_nutrient_defaults() -> Dict[str, List[str]]:
    profiles = get_nutrient_profiles()
    defaults = {key: [] for key in profiles}
    try:
    ...
    for key in defaults:
        ...
        if isinstance(values, list):
            defaults[key] = unique_clean(values)
    return defaults


def save_nutrient_defaults(defaults: Dict[str, List[str]]) -> None:
    profiles = get_nutrient_profiles()
    safe_defaults = {key: unique_clean(defaults.get(key, [])) for key in profiles}
    NUTRIENT_DEFAULTS_PATH.write_text(json.dumps(safe_defaults, ensure_ascii=False, indent=2), encoding="utf-8")
...
def nutrient_selectors(product_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    selected_by_profile: Dict[str, List[str]] = {key: [] for key in profiles}
    ...
    try:
        ...
        saved_defaults = {key: [] for key in profiles}

    columns = st.columns(len(profiles))
    product_widget_suffix = safe_filename(
        clean_text(product_data.get("product_id", ""))
        or clean_text(product_data.get("product_name", ""))
        or "producto"
    )[:48]

    for column, (profile_key, profile_label) in zip(columns, profiles.items()):
        with column:
            ...
            widget_key = f"select_nutrients_{profile_key}_{product_widget_suffix}"
            ...
            selected = st.multiselect(
                ...
                default=fallback,
                key=widget_key,
                help="Selecciona los nutrientes que aparecerán en este tipo de ficha.",
            )
            selected = [x for x in selected if x in available]
            selected_by_profile[profile_key] = selected

            if st.button("Guardar nutrientes", key=f"save_nutrients_{profile_key}_{product_widget_suffix}"):
                saved_defaults[profile_key] = selected
                try:
                    save_nutrient_defaults(saved_defaults)
                    st.success("Nutrientes guardados como defecto para próximos usos.")
                except Exception as exc:
                    st.error(f"No se han podido guardar los nutrientes: {exc}")
...
def main() -> None:
    ...
    try:
        ...
        st.error(f"No se ha podido construir el selector de nutrientes: {exc}")
        profiles = get_nutrient_profiles()
        nutrient_sets = {key: [] for key in profiles}
    doc_selections = field_selectors()

    ctx = build_context(product_display, product_data, manual_values, nutrient_sets)
    product_title = clean_product_name(field_value(ctx, "Nombre comercial"), product_display) or product_display
