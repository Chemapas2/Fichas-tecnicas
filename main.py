...
# Perfiles persistentes y fuentes maestras.
NUTRIENT_PROFILES = {
...
QUALITY_OPERATION_DEFAULTS_PATH = Path("quality_operation_defaults.json")
...
def load_json_defaults(path: Path) -> Dict[str, Any]:
    try:
        if path.exists():
...
def save_json_defaults(path: Path, data: Dict[str, Any]) -> None:
...
def load_parameter_file_bytes(file_bytes: bytes, file_name: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def load_parameter_file(uploaded_file) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if uploaded_file is None:
        return pd.DataFrame(), pd.DataFrame()
    return load_parameter_file_bytes(uploaded_file.getvalue(), uploaded_file.name)


def load_parameter_file_from_path(path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if not path.exists():
        return pd.DataFrame(), pd.DataFrame()
    return load_parameter_file_bytes(path.read_bytes(), path.name)
...
def parse_multimix_analysis(block: str) -> pd.DataFrame:
    ...
    for line in lines[start:]:
        ...
        if len(parts) >= 2 and is_numeric_text(parts[1]):
        ...
        elif len(parts) >= 3 and not is_numeric_text(parts[1]) and is_numeric_text(parts[2]):
            idx = 3
            restriction = ""
            if len(parts) > idx and norm_key(parts[idx]) in {"min", "max", "fijo", "fix"}:
                restriction = parts[idx]
            ...
            parsed = {
                "name": f"{clean_text(parts[0])} {unit}".strip(),
                "level": parts[2],
                "restriction": restriction,
                "minimum": parts[idx] if len(parts) > idx else "",
                "maximum": parts[idx + 1] if len(parts) > idx + 1 else "",
...
def split_singlemix_blocks(raw_text: str) -> List[str]:
    ...
    for i, match in enumerate(matches):
        ...
        if "INCLUDED RAW MATERIALS" in block.upper() or "NUTRIENT ANALYSIS" in block.upper():
            blocks.append(block)
    return blocks


def parse_singlemix_sp_line(block: str) -> Tuple[str, str, str]:
    first_lines = "\n".join(block.splitlines()[:8])
    m = re.search(
        r"\bSP\s*:\s*(?P<spec>\S+)\s+(?P<name>.*?)(?:\s+[-0-9.,]+\s*%,|\s+[-0-9.,]+\s+%,)"
        r".*?(?:Recost|Optimal cost)\s*:\s*(?P<cost>[-0-9.,]+)",
        first_lines,
        flags=re.IGNORECASE | re.DOTALL,
    )
    if not m:
        m = re.search(r"\bSP\s*:\s*(?P<spec>\S+)\s+(?P<name>.+)", first_lines, flags=re.IGNORECASE)
    if not m:
        line = next((clean_text(x) for x in block.splitlines() if "SP:" in x.upper()), "Producto sin nombre")
        return "", clean_product_name(line[:120]), ""
    spec = clean_text(m.group("spec"))
    name = clean_product_name(re.sub(r"\s+", " ", clean_text(m.group("name"))).strip(" :"))
    cost = clean_text(m.groupdict().get("cost", ""))
    return spec, name, cost


def parse_material_identity(material_full: str) -> Tuple[str, str]:
    material_code = ""
    material_name = clean_text(material_full)
    m = re.match(r"(?P<code>\d+)\.(?P<name>.+)", material_name)
    if m:
        material_code = m.group("code")
        material_name = m.group("name").strip()
    return material_code, material_name


def parse_singlemix_ingredients(block: str) -> pd.DataFrame:
    lines = block.splitlines()
    start = None
    for i, line in enumerate(lines):
        if "INCLUDED RAW MATERIALS" in line.upper():
    ...
    for line in lines[start:]:
        up = line.upper()
        if "REJECTED RAW MATERIALS" in up or "NUTRIENT ANALYSIS" in up or "OPTIMIZATION" in up:
            break
        raw_line = str(line).rstrip()
        text = clean_text(line)
        if not text or set(text) <= {"-"}:
        ...
        if len(parts) < 4:
            continue
        first_parts = parts[0].split(None, 1)
        if len(first_parts) < 2:
            continue
        line_code, material_full = first_parts[0], first_parts[1]
        pct = parts[1] if len(parts) > 1 else ""
        kilos = parts[2] if len(parts) > 2 else ""
        ...
        extras = parts[4:]
        if extras:
            if norm_key(extras[0]) in {"min", "max", "fijo", "fix"}:
                limit = extras[0]
                minimum = extras[1] if len(extras) > 1 else ""
                maximum = extras[2] if len(extras) > 2 else ""
            else:
                minimum = extras[0]
                maximum = extras[1] if len(extras) > 1 else ""
        material_code, material_name = parse_material_identity(material_full)
        rows.append({
            "Código línea": clean_text(line_code),
            "Código materia prima": material_code,
            "Materia prima": material_name,
            "Coste medio": parse_float(cost),
            "%": parse_float(pct),
            "Kilos": parse_float(kilos),
            "Toneladas": "",
            "Restricción": clean_text(limit),
            "Mínimo": clean_text(minimum),
            "Máximo": clean_text(maximum),
            "Línea original": text,
        })
    return pd.DataFrame(rows)


def parse_singlemix_rejected_materials(block: str) -> pd.DataFrame:
    lines = block.splitlines()
    start = None
    for i, line in enumerate(lines):
        if "REJECTED RAW MATERIALS" in line.upper():
            start = i + 1
            break
    if start is None:
        return pd.DataFrame()
    ...
    for line in lines[start:]:
        up = line.upper()
        if "NUTRIENT ANALYSIS" in up or "OPTIMIZATION" in up or "INCLUDED RAW MATERIALS" in up:
            break
        raw_line = str(line).rstrip()
        text = clean_text(line)
        if not text or set(text) <= {"-"}:
        ...
        rows.append({
            ...
            "%": "",
            "Restricción": "",
            "Mínimo": clean_text(parts[-2]),
            "Máximo": clean_text(parts[-1]),
            "Línea original": text,
        })
    return pd.DataFrame(rows)
...
def parse_singlemix_text(raw_text: str) -> Dict[str, Dict[str, Any]]:
    ...
    for idx, block in enumerate(split_singlemix_blocks(raw_text), start=1):
        spec, name, cost = parse_singlemix_sp_line(block)
        if not name:
            name = f"Producto {idx}"
        ingredients = parse_singlemix_ingredients(block)
        nutrients = parse_multimix_analysis(block)
        rejected_limits = parse_singlemix_rejected_materials(block)
        nutrient_limits = nutrients[
            nutrients[["Mínimo", "Máximo", "Restricción"]].fillna("").astype(str).agg("".join, axis=1).str.strip() != ""
        ].copy() if not nutrients.empty else pd.DataFrame()
        ingredient_limits = ingredients[
        ...
        if rejected_limits is not None and not rejected_limits.empty:
            ingredient_limits = pd.concat([ingredient_limits, rejected_limits], ignore_index=True) if not ingredient_limits.empty else rejected_limits.copy()
        display = f"{spec} | {name}" if spec else name
        base_display = display
        n = 2
        while display in products:
        ...
        products[display] = {
            ...
            "product_id": spec,
            "cost_per_tonne": cost,
            "source_type": "Single-Mix/SP texto",
            "row": {},
            "nutrients": nutrients,
            "ingredients": ingredients,
...
def extract_products_from_tables(raw_sheets: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, Any]]:
    ...
    for sheet_name, raw_df in raw_sheets.items():
        ...
        if df.empty:
            continue
        flat_text = "\n".join(clean_text(v) for v in df.astype(str).values.ravel().tolist())
        if df.shape[1] <= 2 and ("INCLUDED RAW MATERIALS" in flat_text.upper() or "NUTRIENT ANALYSIS" in flat_text.upper()):
            continue
...
def build_context(
    ...
    if isinstance(selected_nutrients, dict):
        nutrients_commercial = selected_nutrients_df(product_data, selected_nutrients.get("commercial", []))
        nutrients_technical = selected_nutrients_df(product_data, selected_nutrients.get("technical", []))
        ctx["Nutrientes FT Comercial"] = table_to_text(nutrients_commercial)
        ctx["Nutrientes FT Calidad-Operaciones y Especificaciones"] = table_to_text(nutrients_technical)
        ...
        ctx["Nutrientes FT Comercial"] = table_to_text(nutrients)
        ctx["Nutrientes FT Calidad-Operaciones y Especificaciones"] = table_to_text(nutrients)
...
def build_document_text(
    ...
    if include_nutrients:
        ...
        nutrient_text = field_value(ctx, nutrient_key) or field_value(ctx, "Nutrientes seleccionados")
        if nutrient_text:
...
def init_state() -> None:
    st.session_state.setdefault("selected_nutrients_commercial", [])
    st.session_state.setdefault("selected_nutrients_technical", [])
    st.session_state.setdefault("generated_docs", {})
    st.session_state.setdefault("batch_rows", [])


def reset_current_product_state() -> None:
    preserve = {"selected_nutrients_commercial", "selected_nutrients_technical", "batch_rows"}
...
def sidebar_uploads() -> Tuple[Dict[str, Dict[str, Any]], pd.DataFrame, pd.DataFrame]:
    ...
    param_file = None
    use_master_texts = MASTER_TEXTS_PATH.exists()
    if use_master_texts:
        st.sidebar.caption(f"Textos maestros cargados desde `{MASTER_TEXTS_PATH}`.")
        if st.sidebar.checkbox("Cargar otro Excel de etiquetas/beneficios", value=False):
    ...
    elif use_master_texts:
        with st.spinner("Leyendo textos maestros..."):
            etiquetas, beneficios = load_parameter_file_from_path(MASTER_TEXTS_PATH)
        st.sidebar.success(
            f"Textos maestros · Etiquetas: {len(etiquetas)} filas · Beneficios: {len(beneficios)} filas"
        )
...
def select_base_fields(
    ...
    quality_defaults = load_json_defaults(QUALITY_OPERATION_DEFAULTS_PATH)

    def extra_default(label: str, *row_keys: str, fallback: str = "") -> str:
        for row_key in row_keys:
            value = clean_text(row_data.get(row_key, ""))
            if value:
                return value
        return clean_text(quality_defaults.get(label, fallback))
    ...
    if st.button("Pasar opciones elegidas a campos editables", key="apply_proposed_values"):
        apply_proposed_values_to_session(proposed_values)
        st.success("Opciones copiadas a los campos editables.")
        st.rerun()
    ...
    with st.expander("Campos adicionales de Calidad-Operaciones y Especificaciones", expanded=False):
        ...
        with c1:
            ...
            version = st.text_input("Versión", value=extra_default("Versión", "Versión", "Version", fallback="1"), key="manual_version")
            codigo_unite = st.text_input("Código Unite", value=extra_default("Código Unite", "Código Unite", "Codigo Unite", fallback=clean_text(product_data.get("product_id", ""))), key="manual_codigo_unite")
            ean = st.text_input("EAN", value=extra_default("EAN", "EAN"), key="manual_ean")
            presentacion = st.text_input("Presentación", value=extra_default("Presentación", "Presentación", "PRESENTACION"), key="manual_presentacion")
            peso_saco = st.text_input("Peso del producto en saco", value=extra_default("Peso del producto en saco", "Peso del producto en saco"), key="manual_peso_saco")
            homologado = st.text_input("Homologado", value=extra_default("Homologado", "Homologado", "HOMOLOGADO"), key="manual_homologado")
            medicado = st.text_input("Medicado", value=extra_default("Medicado", "Medicado", "MEDICADO"), key="manual_medicado")
            periodo_espera = st.text_input("Periodo de espera", value=extra_default("Periodo de espera", "Periodo de espera"), key="manual_periodo_espera")
            fabrica = st.text_input("Fábrica", value=extra_default("Fábrica", "Fábrica", "FABRICA"), key="manual_fabrica")
            direccion = st.text_input("Dirección", value=extra_default("Dirección", "Dirección", "DIRECCION"), key="manual_direccion")
        with c2:
            codigo_postal = st.text_input("Código postal", value=extra_default("Código postal", "Código postal", "CODIGO POSTAL"), key="manual_codigo_postal")
            poblacion = st.text_input("Población", value=extra_default("Población", "Población", "POBLACION"), key="manual_poblacion")
            provincia = st.text_input("Provincia", value=extra_default("Provincia", "Provincia", "PROVINCIA"), key="manual_provincia")
            telefono = st.text_input("Teléfono", value=extra_default("Teléfono", "Teléfono", "TELEFONO"), key="manual_telefono")
            imagen_envase = st.text_input("Imagen envase", value=extra_default("Imagen envase", "Imagen envase"), key="manual_imagen_envase")
            ficha_envase = st.text_input("Ficha técnica del envase", value=extra_default("Ficha técnica del envase", "Ficha técnica del envase"), key="manual_ficha_envase")
            formato_pale = st.text_area("Formato de palé. Mosaico, alturas y peso", value=extra_default("Formato de palé. Mosaico, alturas y peso", "Formato de palé. Mosaico y alturas y peso", "Formato de palé. Mosaico, alturas y peso"), height=70, key="manual_formato_pale")
            dur_min = st.text_input("Durabilidad mínima %", value=extra_default("Durabilidad mínima %", "Durabilidad minima %", "Durabilidad mínima %"), key="manual_dur_min")
            dur_alert = st.text_input("Alerta durabilidad %", value=extra_default("Alerta durabilidad %", "Alerta durabilidad %"), key="manual_dur_alert")
            finos_max = st.text_input("Finos máximos %", value=extra_default("Finos máximos %", "Finos máximos %"), key="manual_finos_max")
            finos_alert = st.text_input("Alerta finos %", value=extra_default("Alerta finos %", "Alerta finos %"), key="manual_finos_alert")

        definicion = st.text_area("Definición / Posicionamiento", value=extra_default("Definición / Posicionamiento", "Definición / Posicionamiento"), height=100, key="manual_definicion")
        caracteristicas = st.text_area("Características", value=extra_default("Características", "Características"), height=100, key="manual_caracteristicas")
        caracteristicas_nut = st.text_area("Características nutricionales", value=extra_default("Características nutricionales", "Características nutricionales"), height=90, key="manual_caracteristicas_nut")
        materias = st.text_area("Materias primas, aditivos y correctores específicos", value=extra_default("Materias primas, aditivos y correctores específicos", "Materias primas, aditivos, correctores ESPECIFICOS", "Materias primas, aditivos y correctores específicos"), height=90, key="manual_materias")
        analiticas = st.text_area("Analíticas especiales además del plan analítico", value=extra_default("Analíticas especiales además del plan analítico", "Analíticas especiales además de las ya definidas por plan analítico."), height=70, key="manual_analiticas")
        if st.button("Guardar campos adicionales por defecto", key="save_quality_operation_defaults"):
            save_json_defaults(QUALITY_OPERATION_DEFAULTS_PATH, {
                "Fecha": fecha,
                "Versión": version,
            ...
            st.success(f"Información guardada en {QUALITY_OPERATION_DEFAULTS_PATH}.")
...
def nutrient_selectors(product_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    selections: Dict[str, List[str]] = {"commercial": [], "technical": []}

    if not available:
        st.info("No se han detectado nutrientes numéricos en el producto seleccionado.")
        return selections

    saved_defaults = load_json_defaults(NUTRIENT_DEFAULTS_PATH)
    tabs = st.tabs([NUTRIENT_PROFILES["commercial"], NUTRIENT_PROFILES["technical"]])
    for profile_key, tab in zip(["commercial", "technical"], tabs):
        label = NUTRIENT_PROFILES[profile_key]
        with tab:
            session_key = f"selected_nutrients_{profile_key}"
            previous = [x for x in st.session_state.get(session_key, []) if x in available]
            saved = [x for x in saved_defaults.get(profile_key, []) if x in available]
            default = previous or saved or default_nutrients(available)
            selected = st.multiselect(
                f"Nutrientes para {label}",
                options=available,
                default=default,
                help="Puede guardar esta selección para que aparezca por defecto en siguientes usos.",
                key=f"select_nutrients_{profile_key}",
            )
            st.session_state[session_key] = selected
            selections[profile_key] = selected
            if st.button(f"Guardar nutrientes por defecto - {label}", key=f"save_nutrients_{profile_key}"):
                saved_defaults[profile_key] = selected
                save_json_defaults(NUTRIENT_DEFAULTS_PATH, saved_defaults)
                st.success(f"Nutrientes guardados para {label}.")
            if selected:
                st.text(table_to_text(selected_nutrients_df(product_data, selected)))
