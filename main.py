...
NUTRIENT_DEFAULTS_PATH = Path(__file__).with_name("nutrient_defaults.json")
...
def load_nutrient_defaults() -> Dict[str, List[str]]:
    """Carga los nutrientes guardados en disco y normaliza perfiles conocidos."""
...
def save_nutrient_defaults(defaults: Dict[str, List[str]]) -> Tuple[bool, str]:
    """Guarda en un JSON local los nutrientes por perfil de ficha."""
    safe_defaults = {profile: unique_clean(defaults.get(profile, [])) for profile in NUTRIENT_PROFILES}
    try:
        NUTRIENT_DEFAULTS_PATH.write_text(
            json.dumps(safe_defaults, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        return True, f"Nutrientes guardados en {NUTRIENT_DEFAULTS_PATH.name}."
    except Exception as exc:
        return False, f"No se han podido guardar los nutrientes: {exc}"


def get_saved_nutrient_defaults() -> Dict[str, List[str]]:
    """Devuelve los nutrientes guardados, cargándolos una vez por sesión."""
...
def default_nutrients_for_profile(profile: str, available: List[str], current_key: str) -> List[str]:
    if current_key in st.session_state:
        # Respeta una selección vacía si el usuario la ha dejado explícitamente vacía.
...
def sync_editable_defaults(source_signature: str, proposed_values: Dict[str, str]) -> None:
    """
    Sincroniza campos editables con la opción de etiqueta/beneficio elegida.

    Streamlit conserva el valor anterior de un widget con la misma key. Por eso,
    si cambia la especie, subespecie, lifestage u opción propuesta, actualizamos
    explícitamente el estado de los campos manuales antes de renderizarlos.
    """
...
def select_base_fields(
    ...
    source_signature = "|".join([
        ...
        clean_text(species),
        clean_text(subspecies),
        clean_text(lifestage),
        clean_text(selected_label_idx),
        clean_text(selected_benefit_idx),
    ])
    sync_editable_defaults(
        source_signature,
        {
...
def nutrient_selector(product_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if not available:
        ...
        return {doc_type: [] for doc_type in DOC_NUTRIENT_PROFILE}

    # Sanitiza valores retenidos cuando se cambia a un producto con otros nutrientes.
    # Si las selecciones previas no existen en el nuevo producto, se elimina la key
    # para que entren los valores guardados por defecto.
    for nutrient_key in ("select_nutrients_commercial", "select_nutrients_technical"):
        if nutrient_key not in st.session_state:
            continue
        raw_values = st.session_state.get(nutrient_key, [])
        filtered_values = filter_available(raw_values, available)
        if raw_values and not filtered_values:
            del st.session_state[nutrient_key]
        else:
            st.session_state[nutrient_key] = filtered_values

    st.caption(
        "Hay dos perfiles de nutrientes: uno para FT Comercial y otro para FT Calidad-Operaciones/Especificaciones. "
        "Pueden ser distintos y se pueden guardar como defecto para siguientes productos."
    )

    c1, c2 = st.columns(2)
    with c1:
        commercial_key = "select_nutrients_commercial"
        commercial_kwargs = {
            "label": "Nutrientes para FT Comercial",
            "options": available,
            "key": commercial_key,
            "help": "Nutrientes que se incorporarán cuando se genere la ficha comercial.",
        }
        if commercial_key not in st.session_state:
            commercial_kwargs["default"] = default_nutrients_for_profile("commercial", available, commercial_key)
        selected_commercial = st.multiselect(**commercial_kwargs)
        if st.button("Guardar nutrientes FT Comercial", key="save_nutrients_commercial"):
            ok, msg = set_saved_nutrient_defaults("commercial", selected_commercial)
            st.success(msg) if ok else st.error(msg)

    with c2:
        technical_key = "select_nutrients_technical"
        technical_kwargs = {
            "label": "Nutrientes para FT Calidad-Operaciones y FT Especificaciones",
            "options": available,
            "key": technical_key,
            "help": "Nutrientes que se incorporarán en las fichas técnica-operativa y de especificaciones.",
        }
        if technical_key not in st.session_state:
            technical_kwargs["default"] = default_nutrients_for_profile("technical", available, technical_key)
        selected_technical = st.multiselect(**technical_kwargs)
        if st.button("Guardar nutrientes FT Técnica/Especificaciones", key="save_nutrients_technical"):
            ok, msg = set_saved_nutrient_defaults("technical", selected_technical)
            st.success(msg) if ok else st.error(msg)

    return {
        "FT Comercial": selected_commercial,
        "FT Calidad-Operaciones": selected_technical,
        "FT Especificaciones": selected_technical,
    }
...
def main() -> None:
    ...
    if st.session_state.get("_active_product_display") != product_display:
        reset_current_product_state()
        st.session_state["_active_product_display"] = product_display
    ...
    selected_nutrient_sets = nutrient_selector(product_data)
    doc_selections = field_selectors()

    product_title = clean_text(manual_values.get("Nombre comercial", "")) or product_display
        ...
        with col:
            if st.button(f"Generar {doc_type}", key=f"generate_{doc_type}", use_container_width=True):
                ...
                ctx_doc = build_context(
                    ...
                    product_data,
                    manual_values,
                    selected_nutrient_sets.get(doc_type, []),
                )
