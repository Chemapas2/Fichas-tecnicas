...
def save_nutrient_defaults(defaults: Dict[str, List[str]]) -> None:
    safe_defaults = {key: unique_clean(defaults.get(key, [])) for key in NUTRIENT_PROFILES}
...
def nutrient_selectors(product_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    selected_by_profile: Dict[str, List[str]] = {key: [] for key in NUTRIENT_PROFILES}
    ...
    c1, c2 = st.columns(2)
    column_by_profile = {"commercial": c1, "technical": c2}

    for profile_key, profile_label in NUTRIENT_PROFILES.items():
        with column_by_profile[profile_key]:
            ...
            current = [x for x in st.session_state.get(widget_key, fallback) if x in available]
            st.session_state[widget_key] = current or fallback
            ...
            if st.button("Guardar nutrientes", key=f"save_nutrients_{profile_key}"):
